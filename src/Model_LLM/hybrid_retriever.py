...
# RERANK_CANDIDATES = 80     # số doc đem đi rerank
# RERANK_TOP_K = TOP_K       # trả về top-k sau rerank
...
# def _split_text(text: str, chunk_size=900, overlap=250) -> List[str]:
...
#     """
#     1) Thử nạp corpus.jsonl sẵn có (nhanh).
#     2) Nếu chưa có, đọc ./src/Data/*.txt, split chunk và lưu corpus.jsonl cho lần sau.
#     """
...
#         t = _clean_text(text)
#         if not t: continue
#         for piece in _split_text(t, chunk_size=1000, overlap=250):
#             out_docs.append(Document(page_content=piece, metadata={"source": fname, "page": -1, "chunk_id": chunk_id}))
#             chunk_id += 1
