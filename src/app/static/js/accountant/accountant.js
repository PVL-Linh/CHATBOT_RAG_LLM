...
(() => {
    ...
    (function () {
        ...
        if ('ResizeObserver' in window) new ResizeObserver(() => updateOnce()).observe(sidebar)
        new MutationObserver(() => trackFor(getAnimWindowMs())).observe(document.body, { attributes: true, attributeFilter: ['class'] })

        if (mqDesktop.addEventListener) mqDesktop.addEventListener('change', () => trackFor(getAnimWindowMs()))
