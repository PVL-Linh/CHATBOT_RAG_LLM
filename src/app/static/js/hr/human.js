...
(() => {
  ...
  const API = {
    ...
    history: {
      ...
      bySession: (id) => `/api/history/by_session?session_id=${encodeURIComponent(id)}`,
      sessions: (limit) => `/api/history/sessions?limit=${Number(limit) || 200}`,
  ...
  function _renderBlock(block) {
    ...
    const raw = (window.marked ? marked.parse(block.content || "") : block.content || "").toString();
