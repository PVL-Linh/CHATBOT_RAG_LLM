// Complete Chat System - Fixed Session Management (HR API version)
(() => {
  "use strict";
  ...
  const API = {
    chat: "/api/hr/chat",
    chatStream: "/api/hr/chat/stream", // nếu muốn stream sau này
    history: {
      newSession: "/api/history/new_session",
      rename: "/api/history/rename_session",
      delete: "/api/history/delete_session",
      bySession: (id) =>
        `/api/history/by_session?session_id=${encodeURIComponent(id)}`,
      sessions: (limit) =>
        `/api/history/sessions?limit=${Number(limit) || 200}`,
      current: "/api/history/current_session",
    },
  };
  ...
  async function createNewSession() {
    ...
    // Try API directly
    try {
      const r = await fetch(API.history.newSession, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
