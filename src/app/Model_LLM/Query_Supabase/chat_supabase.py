# app.py  (module Database Services)
# -*- coding: utf-8 -*-
...
# ====== External deps (API layer team DB) ======
from app.Model_LLM.Query_Supabase import api_client as ac
from app.Model_LLM.Query_Supabase.api_client import (
    fetch_payment_range, sum_revenue_from_payments, group_revenue_by_staff,
    fetch_staff_by_id, fetch_account_by_id, fetch_orders_range, fetch_order_by_code,
    group_orders_count_by_staff, agg_cashflow_daily, agg_revenue_by_route,
    agg_revenue_by_destination, agg_aov_by_customer, agg_cancel_rate,
    agg_avg_fulfillment_time, agg_feedback_avg_by_customer,
    agg_feedback_negative_customers, agg_feedback_count, agg_repeat_customer_rate,
    agg_outstanding_amount, agg_ship_payment_report, agg_payment_count_by_status,
    list_orders_by_status, list_wait_to_buy, agg_inventory_by_warehouse,
    agg_avg_weight_in_stock, agg_imported_vn_in_period, agg_flights_waiting_count,
    agg_flight_completion_rate, agg_avg_transit_time_foreign_to_vn,
    sum_amount_by_payment_type
)
...
def _fmt_money(v: float) -> str:
    return f"{v:,.0f}₫" if (CURRENCY or "").upper() == "VND" else f"{v:,.2f} {CURRENCY}"
...
def _quarter_range(y: int, q: int, inclusive=True) -> Tuple[str, str]:
    ...
    e_ex = dt.datetime(y + 1, 1, 1, tzinfo=VN_TZ) if q == 4 else dt.datetime(y, start_month + 3, 1, tzinfo=VN_TZ)
    if inclusive:
...
def _resolve_owner(staff_id: Optional[int]) -> Tuple[Optional[str], Optional[str]]:
    ...
    try:
        ...
        scode = (stf.get("staff_code") or "").strip() or None
        return (name if name else None), scode
    except Exception:
        return None, None
...
def _resolve_order_codes_for_payments(pays: list[dict]) -> dict[str, list[str]]:
    ...
    for p in pays:
        ...
        day = None
        try:
            d = dt.datetime.fromisoformat(str(dt_raw))
...
def _call_group_orders_count_by_staff(dt_from: str, dt_to: str):
    for call in (
        ...
        lambda: group_orders_count_by_staff(dt_from),
        lambda: group_orders_count_by_staff(),
    ):
        try:
            res = call()
            if res is not None:
...
def handle_order_count_by_status(period: Dict[str, Any], status: str) -> Dict[str, Any]:
    ...
    if series3:
        ...
        table = {"filename": f"orders_{status.lower()}_daily.csv",
                 "data": {"columns": ["date", "count", "order_codes"], "rows": series3}}
    else:
        breakdown, table = "", None

    label = _fmt_period_label(period)
    reply = f"📊 **Số đơn {label_vn or status} {label}**: **{total}**\nKhoảng: `{dt_from}` → `{dt_to}`{breakdown}"
...
def handle_list_orders_by_status(period: Dict[str, Any], status: str) -> Dict[str, Any]:
    ...
    if not rows:
        return _ok_reply(f"Không có đơn **{_status_vn_label(status) or status}**.")
    ...
    head = [f"- {rr[0]} · {rr[1]} · {(rr[2] or '')}" for rr in table_rows[:5]]
...
def handle_order_count_by_period(period: Dict[str, Any]) -> Dict[str, Any]:
    dt_from, dt_to = _period_to_range(period, inclusive_for_month=False)
    try:
        orders = fetch_orders_range(dt_from, dt_to) or []
    except Exception:
        orders = []
    ...
    if series3:
        ...
        breakdown = f"\n\nPhân rã theo ngày ({len(series3)} dòng):\n{daily_lines}"
        table = {"filename": "orders_count_daily.csv",
                 "data": {"columns": ["date", "count", "order_codes"], "rows": series3}}
    else:
        breakdown, table = "", None

    label = _fmt_period_label(period)
    reply = f"📊 **Tổng số đơn {label}**: **{total}**\nKhoảng: `{dt_from}` → `{dt_to}`{breakdown}"
    return _ok_reply(reply, table, None, meta={"no_summarize": bool(series3)})
...
def handle_revenue_total_by_period(period: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if series:
        chart = {"kind":"line","x":[d for d,_ in series],"y":[v for _,v in series],
                 "title":f"Doanh thu theo ngày – {label}"}
...
def handle_message(user_text: str, session_id: str, db_path: str) -> Dict[str, Any]:
    ...
    if intent == "SMALLTALK":
        ...
        return {
            "intent": intent,
            "reply": "Mình hỗ trợ: doanh thu, số đơn (lọc trạng thái/thể loại), Top theo số đơn, tra mã đơn + follow-up 'của ai' / 'tên gì'."
        }
    ...
    if intent in ("GET_ORDER_STATUS_BY_CODE","GET_ORDER_STATUS"):
        ...
        if staff_id is not None:
            name, scode = _resolve_owner(int(staff_id))
            if name:
    ...
    # ===== AOV / Count by status / Cancel rate =====
