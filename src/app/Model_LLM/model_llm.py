...
def LLM_model(
    ...
    # Default to REMOTE (1) to avoid local download as requested
    force_remote_env = (os.getenv("EMBED_FORCE_REMOTE", "1").strip().lower())
    ...
    if use_remote:
        ...
        if hf_token:
            ...
            # Fallback to Google Gemini Embeddings (Online, No HF Token needed if Gemini Key exists)
            gemini_key = os.getenv("GEMINI_API_KEY") or os.getenv("GOOGLE_API_KEY")
            if gemini_key:
                print("[model_llm] Thiếu HF_TOKEN -> Fallback sang Google Gemini Embeddings (embedding-001).")
                try:
                    ...
                    print("[model_llm] Thiếu libraries cho Google Embeddings (langchain-google-genai). Reverting to local download.")
                    local_embed_dir = ensure_local_hf_model(
                        local_dir=os.environ.get("EMBED_MODEL_DIR", "intfloat/multilingual-e5-small"),
                        model_id=EMBED_MODEL_ID,
                        revision=EMBED_REVISION,
                    )
                    embeddings = _make_embeddings_local(local_embed_dir)
            else:
                # No tokens available, force local download
                print("[model_llm] Thiếu HF_TOKEN và GEMINI_API_KEY -> Buộc phải tải model về local.")
                local_embed_dir = ensure_local_hf_model(
                    local_dir=os.environ.get("EMBED_MODEL_DIR", "intfloat/multilingual-e5-small"),
                    model_id=EMBED_MODEL_ID,
                    revision=EMBED_REVISION,
    ...
    # ====== Vector store: FAISS local hoặc Pinecone ======
    vectordb_type = (os.getenv("VECTORDB_TYPE", "faiss") or "faiss").strip().lower()

    if vectordb_type == "pinecone":
        # Sử dụng Pinecone làm VectorDB (đã được build index bằng Embedding_FAISS.py)
        if not PineconeVectorStore or not Pinecone:
            raise RuntimeError(
                "VECTORDB_TYPE=pinecone nhưng thiếu langchain-pinecone hoặc pinecone-client. "
                "Hãy cài: pip install langchain-pinecone pinecone-client"
            )
        ...
        if not api_key or not index_name:
            raise RuntimeError(
                "VECTORDB_TYPE=pinecone nhưng thiếu PINECONE_API_KEY hoặc PINECONE_INDEX_NAME trong biến môi trường."
            )
        ...
        if index_name not in existing_indexes:
            raise RuntimeError(
                ...
                "Hãy chạy script build index (Embedding_FAISS.py) hoặc tạo index này trước."
            )
        ...
        print(f"[model_llm] ✅ Đang dùng Pinecone index: {index_name}")
    else:
        # Mặc định: dùng FAISS local như cũ
        ...
        try:
            ...
            pass
        print(f"[model_llm] ✅ Đang dùng FAISS local: {faiss_load_dir}")
