...
def _contents_to_messages(contents):
    """Convert Gemini-style contents [{"role":"user","parts":[{"text":...}]}] to LangChain messages."""
...
def _safe_gemini_generate(gclient, model, contents, config, retries=3, backoff=0.4):
    ...
    max_out = (
        ...
        else getattr(config, "max_output_tokens", 4096)
    )
    tokens_est = _estimate_from_contents(contents, max_out)
    last_err: Optional[Exception] = None
    # Check key before checking model as requested
    use_groq = bool(os.getenv("GROQ_API_KEY")) and (os.getenv("LLM_PROVIDER") or "").strip().lower() == "groq"
    ...
    for i in range(retries + 1):
        try:
            ...
            with LLM_SEM:
                if use_groq:
                    ...
                    temperature = (
                        ...
                        if isinstance(config, dict)
                        else getattr(config, "temperature", 0.6)
                    )
                    llm = ChatGroq(
                        model=groq_model,
            ...
            last_err = e
            s = str(e).lower()
            if ("429" in s or "quota" in s or "resource_exhausted" in s or "rate" in s) and i < retries:
                limiter.on_429()
...
def _ensure_groq_ready():
    """Lazy init for Groq to fix Pydantic errors only once."""
    global _GROQ_READY
    if _GROQ_READY:
        return
    try:
        ...
        print(f"[WARNING] Could not rebuild ChatGroq model: {e}")
