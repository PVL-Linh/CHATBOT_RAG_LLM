...
# # Intent nội bộ dành riêng cho SALE (dò bằng rule trên text)
# MY_SALE_INTENTS = {
#     "my_orders_month",
...
#         "account_id": 5,
#         "username": "user1",
#         "role": "SALE" | "LEAD_SALE" | "MANAGER" | "ADMIN" | "STAFF_SALE" | ...,
#         "staff_id": 10,       # nếu có
#         ...
#     }
#     """
...
#     # id "chủ" của user (account_id)
#     account_id = p.get("account_id") or p.get("id")

#     # id nhân viên sale (nếu bạn có staff_id riêng)
#     sale_owner_id = (
#         p.get("staff_id")
...
#     # NEW: route_ids cho LEAD_SALE (dựa vào account_route)
#     route_ids: List[int] = []
#     if account_id:
#         try:
...
#         "route_ids": route_ids,  # quan trọng cho LEAD_SALE
#     }
...
# ) -> List[Dict[str, Any]]:
#     """
#     SALE chỉ thấy payment của đơn gắn với staff_id = sale_owner_id.
#     ADMIN/MANAGER hoặc không có sale_owner_id → thấy tất cả.

#     Giả định payment có field 'staff_id' (hoặc bạn map staff_id của order vào trước).
#     """
#     role = scope.get("role")
#     sale_owner_id = scope.get("sale_owner_id")
#     if role not in {ROLE_SALE} or sale_owner_id is None:
#         return pays
...
# ) -> List[Dict[str, Any]]:
#     """
#     - ADMIN / MANAGER -> thấy toàn bộ orders
#     - SALE            -> chỉ thấy orders với staff_id = sale_owner_id
#     - LEAD_SALE       -> thấy orders có route_id trong scope['route_ids']
#     """
#     role = scope.get("role")
#     sale_owner_id = scope.get("sale_owner_id")
#     route_ids = set(scope.get("route_ids") or [])

#     # ADMIN / MANAGER => không filter
...
#         if role == ROLE_SALE:
#             if sale_owner_id is not None and staff_id_int == sale_owner_id:
#                 out.append(o)

#         elif role == "LEAD_SALE":
#             # Chỉ những đơn đi tuyến mà lead được gán (account_route)
#             if route_id_int is not None and route_id_int in route_ids:
#                 out.append(o)

#         else:
#             # role khác: tuỳ bạn, tạm thời cho qua hết
#             out.append(o)
...
#     sale_owner_id = scope.get("sale_owner_id")
#     if role == ROLE_SALE and sale_owner_id is not None:
#         try:
#             sid = order_or_snap.get("staff_id")
#             if sid is None:
#                 # nếu đơn không gán staff_id → cho xem tạm
#                 return True
#             return int(sid) == int(sale_owner_id)
#         except Exception:
#             return False
...
# # =========================
# # Rule-based intent cho SALE: "các đơn của tôi ..."
# # =========================
...
#     principal: thông tin người dùng (role, staff_id ...) để áp scope SALE.
#     """
#     try:
#         intent, params, conf = classify_query(user_text)
#         if intent not in DB_INTENTS:
#             # vẫn tiếp tục, vì rule-based cho SALE có thể override
#             print(f"[db_router] intent NLU không thuộc DB_INTENTS: {intent}")

#         remember_intent(session_id, intent, conf)
#         scope = _extract_scope(principal)

#         # ==== 0. Rule-based override dành riêng cho SALE ====
...
#                     return (
#                         True,
#                         "⛔ Bạn không có quyền xem đơn này (thuộc nhân viên sale khác).",
#                         {
#                             "intent": intent,
#                             "confidence": conf,
#                             "forbidden": True,
...
#             # RBAC: SALE chỉ nhìn thấy phần của mình
#             pays_paid = _filter_payments_by_scope(pays_paid, scope)
#             pays_unpaid = _filter_payments_by_scope(pays_unpaid, scope)

#             role = scope.get("role")

#             # 🔴 Nếu là SALE mà hoàn toàn không có giao dịch trong kỳ
#             if role == ROLE_SALE and not pays_paid and not pays_unpaid:
#                 msg = (
#                     f"Tài khoản của bạn (nhân viên kinh doanh) hiện không có dữ liệu doanh thu "
#                     f"trong khoảng thời gian từ {lo[:10]} đến {hi[:10]} "
#                     f"hoặc không được phép xem tổng doanh thu của toàn công ty.\n\n"
#                     "Bạn có thể thử các câu hỏi khác phù hợp với quyền hạn của nhân viên kinh doanh, ví dụ:\n"
#                     "- \"Danh sách các đơn của tôi trong tháng này\"\n"
#                     "- \"Những đơn của tôi chưa thanh toán trong tháng này\"\n"
#                     "- \"Trạng thái các đơn tôi đang phụ trách trong tháng này\""
#                 )
...
#             # Chỉ riêng "tháng" mới thêm các bảng bổ sung
#             if intent == "revenue_month":
#                 role_for_month = scope.get("role")

#                 # 3.1. Kho theo tuyến: CHỈ cho ADMIN / MANAGER
#                 if role_for_month in {ROLE_ADMIN, ROLE_MANAGER}:
...
#                         w = _f(r.get("weight"))
#                         name = (
#                             routes.get(int(rid), f"ROUTE#{rid}")
#                             if rid is not None
#                             else "UNKNOWN"
#                         )
#                         kg[name] = kg.get(name, 0.0) + w
#                     rows_kg = [
#                         {"route": k, "kg": round(v, 2)}
...
#                 # 3.2. Trạng thái đơn (SALE: chỉ đơn của mình; ADMIN/MANAGER: toàn bộ)
#                 ords = fetch_orders_range(lo, hi)
#                 ords = _filter_orders_by_scope(ords, scope)
...
#         # RBAC: SALE chỉ thấy doanh thu của chính mình
...
# app/Model_LLM/Chat_Database/db_router.py
# -*- coding: utf-8 -*-
"""
Router xử lý các intent DB (doanh thu, đơn hàng, kho, seller, ...),
có hỗ trợ RBAC theo role:

- ADMIN      : xem toàn công ty
- MANAGER    : xem toàn công ty
- LEAD_SALE  : xem dữ liệu trên các tuyến (route) mà mình phụ trách
- SALE       : chỉ xem dữ liệu gắn với staff_id của chính mình

principal được truyền từ chat.py, gợi ý dạng:
{
    "id": 5,              # account_id
    "account_id": 5,
    "username": "user1",
    "role": "SALE" | "LEAD_SALE" | "MANAGER" | "ADMIN" | ...,
    "staff_id": 10,
    "raw_roles": [...],
}
"""
