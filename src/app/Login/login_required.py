...
#     """Chuẩn hoá biến thể tên quyền về key chuẩn."""
#     t = (tok or "").strip().lower().replace("-", "_")
#     if t in {"managermarketing", "manager_marketing", "manager marketing"}:
#         return "manager_marketing"
#     elif t in {"manager_sales", "managersales", "manager sales"}:
#         return "manager_sales"
#     elif t in {"sale"}:
#         return "sales"  # gộp 'sale' -> 'sales'
#     return t


# def load_users():
#     """Đọc users.csv -> dict users với password_hash/role/roles.
#     - Nếu có cột 'password' mà chưa có 'password_hash' thì tự băm (generate_password_hash) cho tiện chuyển đổi."""
#     users = {}
...
#     # Chuẩn hoá sale->sales ở session nếu có
#     have = {_normalize_role_token(r) for r in have}
#     return have
...
#                     roles TEXT DEFAULT ''  -- vd: "marketing,hr"
#                 );
#             """)
#             con.commit()
#             _DB_READY = True
...
#         return {"password_hash": row["password_hash"], "roles": roles, "role": roles[0] if roles else ""}
#     finally:
#         con.close()
...
#             out.append({"username": r["username"], "roles": roles, "role": roles[0] if roles else ""})
#         return out
#     finally:
#         con.close()
...
# def load_users():
#     """Trả dict {username: {password_hash, password_plain:'', role, roles}} để code cũ không phải sửa nhiều."""
#     users = {}
#     con = _conn(); con.row_factory = sqlite3.Row
#     try:
...
#                 "password_plain": "",        # SQLite không lưu plaintext
