...
# ---------------- HR RAG engine (fallback) ----------------
try:
    from app.rag_hr.engine_hr import answer_with_rag, build_lex_query
except Exception:  # pragma: no cover
    def answer_with_rag(q: str):
        return ("(HR) Engine HR chưa sẵn sàng. Vui lòng cấu hình app.rag_hr.engine_hr", {"hits": []})
    def build_lex_query(q: str):
        return {"lex_query": q, "canonical": q}

# ======================= DATABASE =========================
DB_PATH = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "Data_app", "chat_hr.db"))
...
def init_hr_db() -> None:
    ...
    """)
    cur.execute("""CREATE INDEX IF NOT EXISTS idx_hr_messages_session ON hr_messages(session_id, timestamp)""")
    conn.commit()
    conn.close()
...
# ---------- SQL helpers ----------
def create_hr_session(user: str = "", title: str = "Cuộc trò chuyện HR") -> str:
    sid = str(uuid4())
...
def add_hr_message(session_id: str, role: str, content: str) -> None:
    now = datetime.utcnow().isoformat()
    conn = _get_conn()
    cur = conn.cursor()
    cur.execute(
...
def get_hr_sessions_list(user: str, limit: int = 200) -> List[Dict[str, Any]]:
    conn = _get_conn()
    cur = conn.cursor()
    cur.execute("""
...
def get_hr_session_messages(session_id: str) -> List[Dict[str, Any]]:
    conn = _get_conn()
    cur = conn.cursor()
    cur.execute("SELECT role, content, timestamp AS ts FROM hr_messages WHERE session_id=? ORDER BY timestamp ASC",
                (session_id,))
    out = [dict(r) for r in cur.fetchall()]
    conn.close()
    return out

def rename_hr_session(session_id: str, title: str) -> None:
    conn = _get_conn()
    cur = conn.cursor()
...
def delete_hr_session(session_id: str) -> None:
    conn = _get_conn()
    cur = conn.cursor()
...
def get_latest_hr_session(user: str) -> Optional[Dict[str, Any]]:
    conn = _get_conn()
    cur = conn.cursor()
    ...
    if not row:
        ...
        return None
    sid = row[0]
    messages = get_hr_session_messages(sid)
    conn.close()
...
def _ensure_valid_or_create_session(session_id: str, user: str, title: str) -> str:
    ...
    """
    if not session_id:
        return create_hr_session(user=user, title=title)
...
bp_hr_history = Blueprint("hr_history", __name__, url_prefix="/api/history/hr")

__all__ = ["bp_hr_rag", "bp_hr_history"]
...
def chat_api():
    ...
    user = _get_user()
    try:
        session_id = _ensure_valid_or_create_session(client_sid, user, title="Cuộc trò chuyện HR")
    except PermissionError as e:
        return _json_error(str(e), 403)

    add_hr_message(session_id, "user", user_text)

    t0 = time.time()
    try:
        answer, _trace = answer_with_rag(user_text)
        add_hr_message(session_id, "assistant", answer)
        t1 = time.time()
        return jsonify({
            "ok": True,
            "answer": answer,
...
def chat_stream():
    ...
    def gen():
        ...
        try:
            ...
            add_hr_message(session_id, "assistant", ans)
