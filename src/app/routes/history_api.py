...
# =======================
# Direct Postgres Connection (psycopg2)
# =======================
...
try:
    ...
    print("✅ Kết nối Supabase Postgres thành công trong app!")
except Exception as e:
    raise ConnectionError(f"❌ Không thể kết nối Supabase: {e}")
...
def log_message(
    ...
    try:
        ...
        cur.execute("SELECT title FROM chat_sessions WHERE id = %s AND user_id = %s", (sid, user_id))
        row = cur.fetchone()

        if not row:
            title = content[:50].strip() or "Cuộc trò chuyện" if role == "user" else "Cuộc trò chuyện"
            cur.execute("""
        ...
        cur.execute("""
            INSERT INTO chat_messages (session_id, user_id, role, content, ts_utc, ts_local, model, metadata)
...
def get_session_messages(session_id: str, limit: int = DEFAULT_PAGE_SIZE, offset: int = 0) -> list[dict]:
    user_id = _get_current_user_id()
    if not user_id:
        return []
...
def list_user_sessions() -> list[dict]:
    ...
    try:
        cur.execute("""
            ...
            WHERE s.user_id = %s
            GROUP BY s.id, s.title, s.created_at, s.updated_at, m_last.content, m_last.role
        ...
        for row in rows:
            ...
            if row.get("last_content"):
                ...
                if len(str(row["last_content"])) > 80:
                    text += "..."
                preview = {"role": row.get("last_role"), "content": text}
...
def api_history_by_session():
    ...
    if not req_sid:
        ...
        if user_id:
            ...
            """, (user_id,))
            row = cur.fetchone()
            req_sid = row["id"] if row else create_new_session()["session_id"]
        else:
