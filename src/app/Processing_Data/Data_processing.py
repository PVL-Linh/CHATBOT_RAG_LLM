...
HEADER_TAIL = re.compile(r"(giáo trình|tài liệu|chương|phần|section|chapter|page|trang).{0,40}\b\d{1,3}\b$", re.IGNORECASE)
BULLET = re.compile(r"""^(
    [\-\+\*\u2022\u2023\u25E6\u2219\u00B7\u2043]\s+ |
    \d{1,3}[\.\)]\s+
)$""", re.VERBOSE)
...
def format_line_markdown(ln: str) -> str:
    """
    Chuyển đổi một dòng text thô sang định dạng Markdown cơ bản.
    """
    ln = ln.strip()
    if not ln: 
        return ""
    
    # 1. Xử lý Header: Nếu dòng viết hoa toàn bộ (ngắn) hoặc khớp pattern Header
    # Loại bỏ các dấu chấm cuối câu nếu là header
    is_header = HEADER_TAIL.search(ln) or (ln.isupper() and len(ln) < 100 and not ln.endswith('.'))
    if is_header:
        # Làm sạch header một chút
        clean_ln = re.sub(r"[.:;]$", "", ln)
        return f"\n## {clean_ln}\n"

    # 2. Xử lý List/Bullet: Chuyển mọi bullet point về "- "
    if BULLET.match(ln):
        # Xóa ký tự bullet cũ và thay bằng "- "
        content = re.sub(r'^[\-\+\*\•\.]\s*', '', ln)
        content = re.sub(r'^\d+[\.\)]\s*', '', content) # Nếu muốn giữ số thứ tự thì bỏ dòng này
        return f"- {content}"

    # 3. Xử lý Table (giữ nguyên nếu đã có format bảng, hoặc format sơ bộ)
    # Logic bảng phức tạp hơn, ở mức cơ bản ta giữ nguyên dòng text
    
    return ln

def clean_page_text(raw_text: str, drop_headers=True, keep_tables=True, table_mode="flatten", latex_math=False) -> str:
    """
    Hàm làm sạch chính, nay tích hợp thêm Markdown Formatting.
    """
    ...
    for idx, line in enumerate(lines):
        # 1. Logic lọc Header/Footer cũ (giữ nguyên)
        if drop_headers and _should_should_drop_header_footer(line, idx, total_lines):
            continue
            
        # 2. Chuẩn hóa khoảng trắng
        line = _normalize_spaces(line)
        
        # 3. Format sang Markdown
        md_line = format_line_markdown(line)
        
        output.append(md_line)
        
    return "\n".join(output)

# --- Các hàm phụ trợ cũ giữ nguyên (copy lại để file chạy được) ---
def _should_should_drop_header_footer(line: str, idx: int, total: int) -> bool:
    s = (line or "").strip()
    if not s: return False
    if HR_RULE.fullmatch(s): return True
    if (PAGE_MARK.match(s) or HEADER_TAIL.search(s)) and (idx < 3 or idx > total - 4): return True
    return False

def looks_like_table_cell(s: str) -> bool:
    # (Giữ nguyên logic cũ của bạn ở đây nếu cần dùng cho table extraction)
    return False
