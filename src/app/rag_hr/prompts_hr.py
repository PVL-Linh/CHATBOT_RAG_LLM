...
# # =========================================================
# # 1) PROMPTS — chuyên nghiệp & có quy tắc chào hỏi
# # =========================================================
...
# def get_prompt(conn: sqlite3.Connection, key: str, version: int=None) -> Optional[Tuple[str, dict]]:
#     if version is None:
#         row = conn.execute(
...
# def seed_defaults_if_empty() -> None:
#     """Giữ nguyên hành vi cũ: chỉ seed nếu chưa có bản ghi."""
#     conn = connect()
#     try:
#         row = conn.execute("SELECT 1 FROM prompts WHERE key='system:HR' LIMIT 1").fetchone()
#         if not row:
#             upsert_prompt(conn, key="system:HR", profile="HR",
#                           content=HR_SYSTEM_PROMPT.strip(),
#                           meta={"tone": HR_TONE, "output": HR_OUTPUT})
#         row = conn.execute("SELECT 1 FROM prompts WHERE key='system:GENERIC' LIMIT 1").fetchone()
#         if not row:
#             upsert_prompt(conn, key="system:GENERIC", profile="GENERIC",
#                           content=GENERIC_SYSTEM_PROMPT.strip(),
#                           meta={"lang":"vi"})
#     finally:
#         conn.close()

# def ensure_hr_prompt_latest(force: bool = True) -> int:
#     """
#     Đẩy phiên bản prompt HR mới vào DB (tăng version).
#     - force=True: luôn ghi phiên bản mới.
#     - force=False: chỉ ghi khi nội dung khác bản hiện tại.
#     Trả về version mới (hoặc version hiện tại nếu không ghi).
#     """
#     conn = connect()
#     try:
#         current = get_prompt(conn, "system:HR")
#         if current:
#             content_now, meta_now = current
#             if not force and content_now.strip() == HR_SYSTEM_PROMPT.strip():
#                 # Không cần cập nhật
#                 # Lấy version hiện tại để trả về
#                 cur = conn.execute("SELECT MAX(version) FROM prompts WHERE key='system:HR'")
#                 (ver,) = cur.fetchone()
#                 return ver or 1
#         # Ghi phiên bản mới
#         ver = upsert_prompt(conn,
#                             key="system:HR",
#                             profile="HR",
#                             content=HR_SYSTEM_PROMPT.strip(),
#                             meta={"tone": HR_TONE, "output": HR_OUTPUT})
