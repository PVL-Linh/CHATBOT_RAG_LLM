...
def answer_with_rag(question: str) -> Tuple[str, List[Dict[str, Any]]]:
    ...
    if JD_FULLCOPY_HR and is_jd:
        ...
        if not raw:
            raw = "\n".join(d.page_content for d, _ in ranked if (d.metadata or {}).get("source") == best_src)
        if STRICT_JD_PASSTHRU:
            ans = (raw or "").strip("")
        else:
            ans = strip_citations((raw or "")) if STRIP_CITATIONS_HR else (raw or "")
        trace = _make_trace(ranked)
