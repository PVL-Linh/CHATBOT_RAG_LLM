...
def build_lex_query_accountant(question: str, aliases=None, phrases=None) -> dict:
    q = _norm(question)
    parts = [q] if q else []
    for a in (aliases or []):
        a = _norm(a)
...
def hybrid_retrieve(vs, question: str, lex_query: str = None,
                    k_sem: int = K_SEM_ACCOUNTANT, k_lex: int = K_LEX_ACCOUNTANT, top_k: int = TOP_K_ACCOUNTANT) -> List[Document]:
    q_sem = f"query: {_norm(question)}"
    sem_docs = vs.max_marginal_relevance_search(q_sem, k=k_sem, fetch_k=MMR_FETCH_K_ACCOUNTANT, lambda_mult=MMR_LAMBDA_ACCOUNTANT)

    if FAST_MODE_ACCOUNTANT:
    ...
    merged = sem_docs if not lex_docs else _wrrf_merge(sem_docs, lex_docs, w_a=W_SEM_ACCOUNTANT, w_b=W_LEX_ACCOUNTANT, c=60)
