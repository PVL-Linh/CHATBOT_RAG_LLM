...
def _resolve_path(p: str) -> str:
    ...
    if os.path.isabs(p):
        return p
    base = os.path.dirname(_DOTENV_PATH) if _DOTENV_PATH else os.getcwd()
    return os.path.abspath(os.path.join(base, p))

# ---------- paths / models ----------
INDEX_DIR_HR = _resolve_path(os.environ.get("INDEX_DIR_HR", "./src/app/vectorstore/FAISS_Vector_HR"))
DATA_DIR_HR  = _resolve_path(os.environ.get("DATA_DIR_HR",  "./src/app/Data/HR"))
FAISS_DIR_HR = _resolve_path(os.environ.get("FAISS_DIR_HR", INDEX_DIR_HR))
