...
def _normalize_case(s: str) -> str:
    ...
    return s.lower() if CASE_NORM_HR=="lower" else s.upper()
...
def _load_corpus_jsonl() -> List[Document]:
    ...
    with open(CORPUS_PATH, "r", encoding="utf-8") as f:
        for line in f:
            ...
            if meta.get("norm_case") != CASE_NORM_HR:
                return []
...
def prepare_bm25_docs() -> List[Document]:
    ...
    raw = _scan_txt(DATA_DIR_HR)
    docs, cid = [], 0
    for fname, text in raw.items():
        t = _normalize_case(text)
        for piece in _split(t, 1200, 300):
            docs.append(Document(page_content=piece, metadata={"source": fname, "chunk_id": cid, "norm_case": CASE_NORM_HR}))
            cid += 1
    if docs:
