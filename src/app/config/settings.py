...
class hybrid_retriever:
    TOP_K = int(os.environ.get("TOP_K", "7") or "7")
...
try:
    _rerank_default = os.environ.get("RERANK_TOP_K") or os.environ.get("TOP_K") or str(hybrid_retriever.TOP_K) or "7"
    hybrid_retriever.RERANK_TOP_K = int(_rerank_default)
except ValueError:
    hybrid_retriever.RERANK_TOP_K = 7 
